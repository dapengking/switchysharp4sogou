...
function init() {
    extension = sogouExplorer.extension.getBackgroundPage();
    App = extension.App;
...
function quickSwitchProxy() {
    ...
    if (!Settings.getValue("quickSwitch", false) && typeof(sogouExplorer.flag) == 'undefined')
        return;
...
function refreshTab() {
    if (Settings.getValue("refreshTab", false))
        sogouExplorer.tabs.executeScript(null, { code:"history.go(0);" });
}
...
function addSwitchRule() {
    ...
    // notify 'Options' tabs
    try {
        var tabs = sogouExplorer.extension.getExtensionTabs();
        for (var i in tabs) {
...
function checkRulesFirstTimeUse() {
    if (!Settings.keyExists("rulesFirstTime")) {
        ...
        if (!RuleManager.hasRules()) {
            ...
            sogouExplorer.tabs.create({ url:url });
        }
    }
}

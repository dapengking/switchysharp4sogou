...
var App = sogouExplorer.app.getDetails();
...
function openOptions(firstTime) {
    ...
    var fullUrl = sogouExplorer.extension.getURL(url);
    sogouExplorer.tabs.getAllInWindow(null, function (tabs) {
        for (var i in tabs) { // check if Options page is open already
            if (tabs.hasOwnProperty(i)) {
                var tab = tabs[i];
                if (tab.url == fullUrl) {
                    sogouExplorer.tabs.update(tab.id, { selected:true }); // select the tab
                    return;
                }
            }
        }
        sogouExplorer.tabs.getSelected(null, function (tab) { // open a new tab next to currently selected tab
...
function setIconBadge(text) {
    ...
    //sogouExplorer.browserAction.setBadgeBackgroundColor({ color: [75, 125, 255, 255] });
    sogouExplorer.browserAction.setBadgeBackgroundColor({ color:[90, 180, 50, 255] });
    sogouExplorer.browserAction.setBadgeText({ text:text });
}
...
function setIconTitle(title) {
    ...
    sogouExplorer.browserAction.setTitle({ title:title });
}
...
function setIconInfo(profile, preventProxyChanges) {
    ...
    if (profile.proxyMode == ProfileManager.ProxyModes.direct || profile.proxyMode == ProfileManager.ProxyModes.system) {
        sogouExplorer.browserAction.setIcon({ path:iconInactivePath });
        title += profile.name;
    } else {
        var iconPath = iconDir + "icon-" + (profile.color || "blue") + ".png";
        sogouExplorer.browserAction.setIcon({ path:iconPath });
...
function monitorTabChanges() {
    ...
    sogouExplorer.tabs.onUpdated.addListener(function (tabId, changeInfo, tab) {
        if (changeInfo.status == "complete") {
            sogouExplorer.tabs.getSelected(null, function (selectedTab) {
